#Creating a graph of Water Boards vs CA ethnicities for R5 REWG ----
# resources ----
# census data: 
...
# General:
#   - Tips on GGPlot2: http://www.sthda.com/english/wiki/ggplot2-barplots-quick-start-guide-r-software-and-data-visualization#change-fill-colors
#   - Tips on exporting to PowerPoint: https://www.brodrigues.co/blog/2018-10-05-ggplot2_purrr_officer/
...
# Filtering for Water Boards
df_5112_2020_water_boards <- df_5112_2020 %>% 
    filter(department == "Water Resources Control Board")
...
# Filtering for Water Boards
df_5102_2020_water_boards <- df_5102_2020 %>% 
    filter(dept == "Water Resources Control Board")
...
## 5102 Water Boards summary functions
fun_summary_5102_l1_wb <- function(dataset) {
    dataset %>%
        ...
        mutate(type = 'Water Boards Employees') %>%
        arrange(ethnicity_level1) %>%
        {.}
}

fun_summary_5102_l2_wb <- function(dataset) {
    dataset %>%
...
# z_test_wb <- df_5102_2020_water_boards %>% 
...
#         guides(fill = guide_legend(reverse = TRUE))
# }
# (z_test_plot <- z_test_wb %>% 
#         fun_plot_5102_l2_facet(plot_title = 'test plot'))
# 
# # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

## Water Boards summary ----
### L1 ----
(pl_5102_water_boards_l1 <- df_5102_2020_water_boards %>% 
     fun_summary_5102_l1_wb() %>% 
     bind_rows(acs_data_level1) %>% 
     fun_plot_5102_l1(plot_title = 'Water Boards Employees vs State Population'))

# ggsave('wb_summary.png',
#        pl_5102_water_boards_l1)

### L2 ----
(pl_5102_water_boards_l2 <- df_5102_2020_water_boards %>% 
     fun_summary_5102_l2_wb() %>% 
     bind_rows(acs_data_level2) %>% 
     fun_plot_5102_l2(plot_title = 'Water Boards Employees vs State Population'))

# ggsave('wb_summary2.png',
#        pl_5102_water_boards_l2)

## Legal ----
### L2 ----
(pl_5102_legal_l2_wb <- df_5102_2020_water_boards %>% 
     filter(employee_category == 'Legal Occupations') %>%
     fun_summary_5102_l2_wb() %>% 
     bind_rows(acs_data_level2) %>% 
     fun_plot_5102_l2(plot_title = 'Water Boards Legal Occupations vs State Population'))
...
## Management ----
### L2 ----
(pl_5102_mgmt_l2_wb <- df_5102_2020_water_boards %>% 
     filter(employee_category == 'Management Occupations') %>%
     fun_summary_5102_l2_wb() %>% 
     bind_rows(acs_data_level2) %>% 
     fun_plot_5102_l2(plot_title = 'Water Boards Management Occupations vs State Population'))
...
## Admin ----
### L2 ----
(pl_5102_admin_l2_wb <- df_5102_2020_water_boards %>% 
     ...
     fun_summary_5102_l2_wb() %>% 
     bind_rows(acs_data_level2) %>% 
     fun_plot_5102_l2(plot_title = 'Water Boards Administrative Occupations vs State Population'))
...
## Tech ----
### L2 ----
(pl_5102_tech_l2_wb <- df_5102_2020_water_boards %>% 
     ...
     fun_summary_5102_l2_wb() %>% 
     bind_rows(acs_data_level2) %>% 
     fun_plot_5102_l2(plot_title = 'Water Boards Technical Occupations vs State Population'))

# ggsave('wb_technical.png',
#        pl_5102_tech_l2_wb,
#        width = 7.5)
