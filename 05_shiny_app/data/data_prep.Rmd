...
df_5012_level2 <- df_5102_report_clean %>%
    ...
    mutate(Level = "Level 2",
           type = factor('Department Workforce'))
...
workforce_data_updated <- workforce_data %>%
        filter(
            dept == "Administrative Law, Office of",
            report_year == 2019,
            Level == "Level 1"
        ) %>% 
        ...
        bind_rows(acs_data_both_levels) 
        # %>% 
        # filter(!is.na(Level))
```
...
```{r}
# setwd("05_shiny_app/data")
# write.csv(workforce_data, file = "workforce_data.csv")
write_csv(x = workforce_data, 
          file = "workforce_data.csv.gz")
```
