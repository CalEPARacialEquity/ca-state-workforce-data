...
In 2020, the United States Census Bureau conducted the Decennial Census, the primary purpose of which is congressional apportionment and state redistricting. The ACS is primarily for measuring demographics, which is the purpose of this report. Due to low response rates to the ACS in 2020, the Census Bureau instead released a set of experimental estimates for that year. However, the US Census Bureau urges data users to not compare the 2020 experimental ACS data with other years of ACS data. For this reason, we used the Decennial Census as the official population count for 2020. The Decennial Census uses different survey methods than the ACS, so we still caution direct comparison of demographic data between the two.
...
The ACS was affected by the pandemic. As suggested by [this flow chart by the Census Bureau](https://www.census.gov/programs-surveys/acs/library/flyers/flow-chart.html), we are still using the ACS experimental estimates. 
...
default_pl_caption <- glue('Data sources: state population data from 2021 1 yr American Community Survey  |  workforce data from {report_year} CalHR 5102 Report')
```
